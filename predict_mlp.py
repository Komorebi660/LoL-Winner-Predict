...
class MyDataset(Dataset):
    def __init__(self, split='train'):
        ...
        self.features = dataset.drop(['win'],axis=1).values.astype(np.float32)
        self.labels = dataset['win'].values.astype(np.float32)
...
def train(args, device):
    ...
    # get model
    model = DenseModel(input_dims=39).to(device)
    model.train()
    # get optimizer
...
def inference(model, device):
    ...
    for data in dataloader:
        with torch.no_grad():
            ...
            prediction = (prediction_scores > 0.5).int().cpu().numpy()  # 0 as threshold
            results['win'].extend(prediction.reshape(-1))
